...
- Python with `google-adk`, `litellm`, `pyyaml`, `requests`, `beautifulsoup4`,
  `tavily-python`, `pypdf` installed.
- A model to run the agent on, selected via `MODEL` in [agent.py](agent.py):
  - `MODEL_OPENAI` (current default, `"gpt-5.6-terra"`) — requires
    `OPENAI_API_KEY` and `USE_LITE_LLM = True` (routes through `litellm`,
    which translates the `max_tokens`/`max_completion_tokens` param
    correctly for newer OpenAI models — ADK's native OpenAI integration
    does not).
  - `MODEL_GEMMA4` (`"ollama_chat/gemma4:12b"`) — requires a local Ollama
    server running that model.
...
`root_agent` is given six tools, all defined in [agent.py](agent.py):
...
| `read_file(path, name)` | Read and return a plain-text file's content (e.g. `.md`, `.txt`, `.csv`). |
| `read_pdf_file(path, name)` | Extract and return the text content of a `.pdf` file, page by page, via `pypdf`. |
